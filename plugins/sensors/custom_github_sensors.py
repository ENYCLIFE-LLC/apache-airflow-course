"""
CustomGitHubSensors Uses the `hook` to interact with the GitHub API responses. It has two custom sensors extending the `GitHubSensor` class to check if any pull request has been merged within the last 24 hours and if a specific file has changed in the latest commit within 24 hours.
24 hours is the default time frame, but you can adjust it by changing the `delta_time` and `delta_time_type` variables when you initialize the custom sensor class object.
...
Returning `False` signals the task is not complete and the sensor will continue to poke until it times out.
3. Timeout Handling: Sensors need to handle a timeout gracefully, meaning that they must exit if they have not succeeded within a given time frame.
